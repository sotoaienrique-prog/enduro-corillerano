unit AgregarTomaTiempoAbastecimiento;
...
type
  TAddTomaTiempoAbastecimiento = class(TForm)
    buttonPanel: TPanel;
    mainPanel: TPanel;
    lblNumMoto: TcxLabel;
...
var
  AddTomaTiempoAbastecimiento: TAddTomaTiempoAbastecimiento;

implementation

{$R *.dfm}

procedure TAddTomaTiempoAbastecimiento.FormCreate(Sender: TObject);
begin
  ZEvento.Active:= True;
//  cmbFilterEvento.
end;

procedure TAddTomaTiempoAbastecimiento.txtNumMotoKeyPress(Sender: TObject; var Key: Char);
var
  ...
  if Key = #13 then
    begin
      ...
      begin
        ...
        ELSE
          BEGIN
            TBussinessDataSubsystemFacade.getInstance.addTiempoAbastecimientoPiloto(pilotoOID, cmbFilterEvento.KeyValue, nroVuelta);
...
function TAddTomaTiempoAbastecimiento.validatePiloto(): Boolean;
var
  returnValue : Boolean;
...
procedure TAddTomaTiempoAbastecimiento.setTListItemsByString(errorListView : TcxListView;
    stringMessage : String; indiceEstado : Integer; clear : Boolean);
var
  Index : Integer;
